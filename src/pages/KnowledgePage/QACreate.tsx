...
import { createQAKnowledge } from "@/request/API/knowledge"
...
export default function QACreate({
    ...
    const handleCreate = async () => {
        const { data: { code }} = await createQAKnowledge({ ...knowledge, user_id: user.id })
        if(code !== 200) {
            return toast({title: '创建失败', description: '创建QA知识库失败', variant: 'destructive', duration: 2000})
        }
        setKnowledge(knowledgeInit)
        onConform()
        return toast({title: '创建成功', description: '创建QA知识库成功', duration: 2000})
    }
